<?php
    ...
    abstract class ZurmoModuleController extends ZurmoBaseController
    {
        ...
        protected function composeEmail()
        {
            $modelClassName        = $this->getModelName();
            ...
            if (!$dataProvider)
            {
                $idsToSend = array_filter(explode(",", trim($_GET['selectedIds'], " ,"))); // Not Coding Standard
            }
            $totalItems = $this->getSelectedRecordCountByResolvingSelectAllFromGet($dataProvider, false);
            ...
            if ($totalItems > 0)
            {
                ...
                if ($dataProvider)
                {
                    $recipientsToSend = $dataProvider->getData();
                    foreach ($recipientsToSend as $model)
                    {
                       if (ControllerSecurityUtil::doesCurrentUserHavePermissionOnSecurableItem($model, Permission::READ))
                       {
                           $data[] = $model->primaryEmail;
                       }
                    }
                }
                else
                {
                    foreach ($idsToSend as $idToSend)
                    {
                        $model = $modelClassName::getById(intval($idToSend));
                        if (ControllerSecurityUtil::doesCurrentUserHavePermissionOnSecurableItem($model, Permission::READ))
                        {
                            $data[] = $model->primaryEmail;
                        }
                ...
                {
                    Yii::app()->user->setFlash('notification',
                        Yii::t('Default', 'You need to select recipients to compose the email.')
                    );
                }
            }
            else
            {
                Yii::app()->user->setFlash('notification',
                    Yii::t('Default', 'There is no data to compose email.')
                );
            }
            //TODO: Should we use $_GET to pass the emails to emailMessage or should we setState and getState in the user?
            $this->redirect(Yii::app()->createUrl('emailMessages/default/composeEmail', array('toRecipients' => $data)));
        }
