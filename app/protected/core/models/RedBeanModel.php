<?php
    ...
    abstract class RedBeanModel extends BeanModel implements Serializable
    {
                                         ...
                                         $selectDistinct = false)
        {
            ...
            $tableName = static::getTableName($modelClassName);
        ...
        public static function getForeignKeyName($modelClassName, $relationName)
        {
            ...
            foreach ($metadata as $modelClassName => $modelClassMetadata)
            {
                    ...
                    array_key_exists($relationName, $metadata[$modelClassName]["relations"]))
                {
                    ...
                    $relatedModelTableName = static::getTableName($relatedModelClassName);
                    $linkType              = null;
        ...
        private static function findNextDerivativeBean($bean, $modelClassName1, $modelClassName2)
        {
            ...
            $tableName = static::getTableName($modelClassName2);
            ...
            elseif ($this->isAttribute($attributeName))
            {
                ...
                else
                {
                    if (!array_key_exists($attributeName, $this->relationNameToRelatedModel))
                    {
                        ...
                        $relatedTableName          = static::getTableName($tempRelatedModelClassName);
                        switch ($relationType)
                        {
        ...
        protected function unrestrictedSet($attributeName, $value)
        {
            ...
            elseif (static::isAnAttribute($attributeName))
            {
                ...
                else
                {
                    ...
                    list($relationType, $relatedModelClassName, $owns, $linkType, $relationLinkName) =
                         $relationAndOwns[$attributeName];
                    $relatedTableName = static::getTableName($relatedModelClassName);
                    switch ($relationType)
                    {
                        case self::HAS_MANY:
                        case self::MANY_MANY:
                        ...
                        foreach ($this->relationNameToRelatedModel as $relationName => $relatedModel)
                        {
                            ...
                            if ($relatedModel instanceof RedBeanModel)
                            {
                                    ...
                                    $this->isAttributeRequired($relationName)))
                                {
                                    ...
                                    {
                                        $tableName  = static::getTableName(static::getAttributeModelClassName($relationName));
        ...
        protected static function getByRelatedClassId($relatedModelClassName, $id, $modelClassName = null)
        {
            ...
            $tableName = static::getTableName($relatedModelClassName);
        ...
        public function forget()
        {
            ...
            RedBeansCache::forgetBean(static::getTableName(get_called_class()) . $this->id);
        }
